        ...
        <div class="row q-pa-sm-sm q-pa-md">
          <div class="col-12">
            <q-card-section>
              ...
              <!-- Notice @submit.prevent instead of @submit -->
...
import register from "src/firebase/firebase-register"; // Import the register function

const router = useRouter(); // Capture router instance
console.log("Router in component:", router); // Debugging to check router
...
const submit = async () => {
  if (form.value.validate()) {
    try {
      const registeredUser = await register(user, router); // Pass router here
      Notify.create({
        type: "positive",
        message: `Welcome ${registeredUser.displayName}! Registration successful.`,
      });
    } catch (error) {
      Notify.create({
        type: "negative",
